...
#------------------------------------------------------------------------------
# replace xxx's with the node name
# xmlrpc http://xxx-xxxx.fnal.gov:21000/RPC2 setcalled
# should return
#--------------
# Result:
#
# String: 'set_called returns True'
#------------------------------------------------------------------------------
def test_rpc():
    called   = [False]
    rpc_port = 21000;
    ...
    def set_called():
        called[0] = True
        return f"set_called returns {called[0]}"

    with rpc_server(host='mu2edaq09-ctrl',port=rpc_port,funcs={"setcalled": set_called}):
        with rpc_client(host='mu2edaq09-ctrl',port=rpc_port) as c:
            c.setcalled()
            eq(called, [True])
