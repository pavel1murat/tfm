...
- some (older) documentation may be obsolete
* ------------------------------------------------------------------------------
* <2026-05-17 Sun> how to generate config/scripts/quick_setup.sh for TFM job submission 
#+begin_src
  fn=config/scripts/quick_setup.sh
  declare -p >| $fn
  declare -f >> $fn
#+end_src
* <2026-05-17 Sun> how to generate fcls:                                                
#+begin_src
  config/scripts/gen_artdaq_fcl.sh demo_mc2 
#+end_src
- the script writes into the configuraiton directory - i.e. config/artdaq/demo_mc2  - FCL files
  for enabled processes on enabled nodes
  - artdaq should also be enabled on those nodes
  - if a boardreader DTC is declared but not enabled, the FCL file is not written
* [[file:./communication_with_artdaq.org][communication with artdaq]]
* artdaq subsystems and logic of their communication                         
- a subsystem is a group of processes. There are 5 types of processes:
- boardreaders
- eventbuilders
- dataloggers
- dispatchers
...
the ordering above is hierarchical :

- normally, the boardrearers send data to event builders
- event builders send data to data loggers
- data logggers - to dispatchers.
- dispatchers send data to slow monitoring clients. It is not clear why
  dispatchers are needed at all - the data loggers can communicate with
  the slow control clients w/o any need in additional layers
- routing managers - dunno, guess they route the network traffic across
  multiple subnets, if a DAQ system spans over

a subsystem may have inputs (sources) and outputs (destinations) defined.
In this case, input to the first layer of the subsystem comes from the last
layer of the source subsystem(s), and the output of the last layer
of this subsystem is directed to the lowest layer of the destination subsystem
- multiple sources
- single destination
...
* get rid of Rank in the process description
* ------------------------------------------------------------------------------
* attic                                                                      
