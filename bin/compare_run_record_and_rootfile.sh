...
temporary_tfm_boot_file=/tmp/$(uuidgen)
...
config_dumper -P $rootfile 2> /dev/null | sed -r 's/\\n/\n/g'  | sed -r '1,/boot: "contents/d;/^\s*\\"\s*$/,$d;s/\\"/"/g'  > $temporary_tfm_boot_file 
...
if [[ ! -s $temporary_tfm_boot_file ]]; then
    echo "It appears no Tfm boot info was saved in $rootfile" 
fi
...
if [[ ! -e $run_records_boot_file ]]; then
    echo "Unable to find tfm boot file \"${run_records_boot_file}\"" >&2
    exit 30
fi
...
grep -E -v 'Total events|tfm start time|tfm stop time' $run_records_metadata_file > $cleaned_run_records_metadata_file

res_boot=$( diff --ignore-blank-lines $temporary_tfm_boot_file $run_records_boot_file )
...
if [[ -z $res_boot && -z $res_metadata ]]; then
    ...
    rm -f $temporary_tfm_boot_file $temporary_metadata_file $cleaned_run_records_metadata_file
    exit 0
fi

if [[ -n $res_boot ]]; then
    echo $res_boot
    echo "tfm boot file info inconsistent between $rootfile and $recorddir/$runnum (see above for diff)"
fi
...
echo temporary_tfm_boot_file=$temporary_tfm_boot_file
